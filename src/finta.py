...
class TA:
    ...
    @classmethod
    def SMA(cls, ohlc: DataFrame, period: int = 41, column: str = "price") -> Series:
        """
    ...
    def ZLEMA(cls, ohlc: DataFrame, period: int = 26, adjust: bool = True) -> Series:
        ...
        ema = pd.Series(
            (ohlc["price"] + (ohlc["price"].diff(lag))),
    ...
    @classmethod
    def WMA(cls, ohlc: DataFrame, period: int = 9, column: str = "price") -> Series:
        """
    ...
    @classmethod
    def HMA(cls, ohlc: DataFrame,  period: int = 16, column: str = "price") -> Series:
        """
        ...
        wmaf = cls.WMA(ohlc, period=half_length, column=column)
        wmas = cls.WMA(ohlc, period=period, column=column)
    ...
    def MACD(
        ...
        EMA_fast = pd.Series(
            ohlc["price"].ewm(ignore_na=False, span=period_fast, adjust=adjust).mean(),
            name="EMA_fast",
        )
        EMA_slow = pd.Series(
            ohlc["price"].ewm(ignore_na=False, span=period_slow, adjust=adjust).mean(),
            name="EMA_slow",
        )
