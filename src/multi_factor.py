...
class pe_factor_strategy(bt.Strategy):
    ...
    def find_swap_position(self):
        # get factor
        factor_list = ["peRatio","roe"]

        #init score list
        score_list = []
        for stock in self.pool_list:
            score_list.append([stock,0])
        ...
        for fac in factor_list:
            ...
            for stock in self.pool_list:
                ...
                the_rate = df_date_data(kdf, stock, fac, self.current_date.strftime("%Y-%m-%d"))

                if not math.isnan(pe_rate):
                    result_list.append([stock,the_rate])

            sorted_result_list = sorted(result_list,key=lambda x:x[1])

            for stock in self.pool_list:
                score_list[stock] += len(self.pool_list) - sorted_result_list[stock]
        ...
        sorted_result_list = sorted(score_list,key=lambda x:x[1])

        # to_sell_list=[i[0] for i in score_list[:self.p.portfolio_size]]
        to_buy_list = [i[0] for i in score_list[-self.p.portfolio_size:]]
        return to_buy_list
