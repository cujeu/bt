...
def runstrat(sector_name, bar_day, etf_name=None):
    ...
    if sector_name == 'all':
        ...
        #3.1. my watch 
        ticker_list = get_mywatch_symbols()
        #print(ticker_list)
        g_ema_count = 0
        mk_df = start_scan(ticker_list, start_date, end_date)
        g_newlow_list.append(["<==MW",str(g_ema_count) + "|"+str(end_date)])
        print(g_newlow_list)
        filename = conf_data_path + 'div_mw.csv'
        mk_df.to_csv(filename, encoding='utf8')
...
if __name__ == '__main__':
    ...
    #print(args.etf, bar_day, args.sector)
